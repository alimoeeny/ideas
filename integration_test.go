...
func Test_integration_needMoreInfo(t *testing.T) {
	itIsRaining := Idea{ID: newStrID(), EnglishHumanReadableExpression: "It is raining", Facts: map[string]*Measurement{CONCEPT_WEATHER_IS_RAINING.ID: {ID: newStrID(), Timestamp: time.Now().UnixNano(), Value: true}}}
	itIsNotRaining := Idea{ID: newStrID(), EnglishHumanReadableExpression: "It is not raining", Facts: map[string]*Measurement{CONCEPT_WEATHER_IS_RAINING.ID: {ID: newStrID(), Timestamp: time.Now().UnixNano(), Value: false}}}
	needMoreInfo := Idea{ID: newStrID(), EnglishHumanReadableExpression: "Need More Info", Facts: map[string]*Measurement{CONCEPT_DATA_NEEDED.ID: {ID: newStrID(), Timestamp: time.Now().UnixNano(), Value: true}}}
	...
	c, ok := ideas[0].Facts[CONCEPT_DATA_NEEDED.ID]
	if !(ok && c.Value == true) {
...
func Test_integration_giveMeIdeas(t *testing.T) {
	...
	fmt.Printf("%#v\n", ideas)
	c, ok := ideas[0].Facts[CONCEPT_WEATHER_IS_RAINING.ID]
	if !ok || c.Value == true {
